...
class MusicPlayer:
    ...
    def create_widgets(self):
        ...
        self.cover_label = ctk.CTkLabel(cover_frame, text="🎧", justify="center")
    ...
    def show_playlist_window(self):
        ...
        ctk.CTkButton(btn_frame, text="➕ Add", command=self.open_file).pack(side="left", padx=5)
    ...
    def sync_playlist_position(self):
        if self.playlist_window and self.playlist_window.winfo_exists():
            ...
            y = self.root.winfo_y()
            self.playlist_window.geometry(f"+{x}+{y}")
            self.root.after(200, self.sync_playlist_position)
